...
export default function VentPage() {
  const { user } = useAuth();
  const { amberName, setHeldEmotion } = useUserProfile();
  ...
  const aiName = amberName;
  const amberFlash = useAmberAttention();

  // tab_conversations 저장 헬퍼
  ...
  function sendMsg() {
    ...
    const aiReply = data.questions[nextQIdx][0];
    const aiTone  = data.questions[nextQIdx][1];
    setMsgs(m => [...m, { role: 'user', text: txt }]);
    setMsgCount(newCount);
    setQIdx(nextQIdx);
    saveConversation(txt, aiReply);
    setTimeout(() => {
      setMsgs(m => [...m, { role: 'ai', text: aiReply, tone: aiTone }]);
      if (newCount >= 4) {
        // held_last_emotion 저장
        setHeldEmotion(curEmo);
                ...
                {showSummary && curEmo && (
                  <div className="vr-fade-in flex-shrink-0 rounded-[11px] mt-2" style={{ background: C.bg2, border: `1px solid #D4A57422`, padding: '14px 15px' }}>
                    ...
                    <div className="flex gap-[7px]">
                      ...
                      <button onClick={() => navigate('/home/dig')}
                        className="flex-1 py-[9px] rounded-[9px] text-[11px] font-light cursor-pointer transition-all"
