-- Create brand_strategies table for brand design tool
...
-- Optional FK only to public schema (avoid FK to auth.users per best practices)
alter table public.brand_strategies
  add constraint brand_strategies_analysis_result_fk
...
-- Enable RLS and add secure policies (only authenticated users manage their own rows)
