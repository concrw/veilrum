...
import { WhyAnalysisTrigger } from "@/components/why/WhyAnalysisTrigger";
import { WhyAnalysisResult } from "@/components/why/WhyAnalysisResult";
import { WhyAnalysisExportButton } from "@/components/why/WhyAnalysisExportButton";
import { useLatestAnalysis } from "@/hooks/useWhyAnalysis";
...
const WhyAnalysis = () => {
  ...
  const { data: aiAnalysis, isLoading: aiAnalysisLoading } = useLatestAnalysis();
  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-background text-foreground px-4 pb-24 pt-6">
        <div className="py-16 text-center text-muted-foreground">분석 결과를 불러오는 중...</div>
      </main>
    );
  }
      ...
      <>
        ...
        <main className="min-h-screen bg-background text-foreground px-4 pb-24 pt-6">
          <header className="mb-6">
            ...
            <h1 className="text-lg font-medium">WHY 분석결과</h1>
          </header>
          
          <div className="py-8 text-center text-muted-foreground">
            <p className="text-sm">분류된 직업이 없습니다.</p>
            <p className="text-xs mt-2">먼저 WHY 분석을 완료해주세요.</p>
            <Button 
              variant="outline" 
              size="sm" 
              onClick={() => navigate('/why')} 
        ...
        <ScrollArea className="h-[calc(100vh-200px)]">
          <div ref={contentRef} className="space-y-6">
            ...
            {aiAnalysis && !aiAnalysisLoading && (
              <div className="mb-8">
                <h2 className="text-xl font-bold mb-4">AI 패턴 분석 결과</h2>
                <WhyAnalysisResult analysis={aiAnalysis} />
                <div className="mt-6 pt-6 border-t">
                  <h3 className="text-lg font-semibold mb-4">기본 분석</h3>
                </div>
              </div>
            )}
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-xs text-muted-foreground mb-4">{perspective.desc}</p>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">핵심 인사이트</CardTitle>
              </CardHeader>
              <CardContent>
                    ...
                    <ScrollArea className="h-32">
                      <div className="space-y-2">
                        {byCategory[c].map((job) => (
                          <div key={job.id} className="text-xs border border-border rounded p-2 bg-muted/20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Card>
                <CardHeader>
                  <CardTitle className="text-sm">키워드 클라우드</CardTitle>
                </CardHeader>
                <CardContent>
                  {topKeywords.length === 0 ? (
                    <p className="text-xs text-muted-foreground">분석할 키워드가 충분하지 않습니다.</p>
                  ) : (
                    <div className="flex flex-wrap gap-2">
                      {topKeywords.map((k, idx) => (
                        <Badge 
                          ...
                          className={`text-xs ${idx < 3 ? "bg-primary/80" : ""}`}
                        >
                          {k.word} ({k.count})
                        </Badge>
                      ))}
                ...
                <CardContent>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-xs">감정 점수</span>
                      <span className="font-medium text-xs">{emotion.score}/100</span>
                    </div>
                    <Progress value={emotion.score} className="h-3" />
                    <div className="text-center">
                      <Badge 
                        variant={emotion.score > 60 ? "default" : emotion.score < 40 ? "destructive" : "secondary"}
                        className="text-xs"
                      >
                        {emotion.label}
                      </Badge>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-sm">추천 액션 아이템</CardTitle>
              </CardHeader>
              <CardContent>
              ...
              <CardContent>
                <div className="space-y-3">
                  <p className="text-xs text-muted-foreground">
                    WHY 분석이 완료되었습니다. 이제 Ikigai 설계를 통해 구체적인 방향을 설정해보세요.
                  </p>
                  <Button 
                    size="sm" 
